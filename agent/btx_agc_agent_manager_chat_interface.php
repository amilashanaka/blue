<?php include_once './bluetelecoms/btx-includes.php';?>
<?php
		...
		xmlhttp.onreadystatechange = function() 
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
				...
				if (ChatText_array[1].match(/^CHAT ENDED/)) 
					...
					document.getElementById("invitetochatbutton").style.display = 'none';
		...
		xmlhttp.onreadystatechange = function() 
			...
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
				...
				if (ChatText.length>0 && ChatText.match(/^Error/)) 
					...
					{
					document.getElementById(destinationId).innerHTML=ChatText.replace("<BR><BR><input class='blue_btn' type='button' style='width:100px' value='INVITE' onClick='AddAgentToExistingChat()'>", '');
					$('#'+destinationId).change(function() {update_chat_dropdowns();});
					}
				}
			}
		delete xmlhttp;
...
function update_chat_dropdowns()

{
	...
	if ($("#agent_to_add").val()!='')
		{ 
			$("#invitetochatbutton").prop('disabled', false);
		}
		else
		{ 
			$("#invitetochatbutton").prop('disabled', true);
		}

	 	
	if (($("#agent").val()!='') && ($("#agent_message").val()!=''))
		{
			$("#startnewchat").prop('disabled', false);
		}
		else
		{ 
			$("#startnewchat").prop('disabled', true);
		}
...
function mutealertcheckbox() {
        if($('#MuteChatAlert').prop( "checked") == true) 
        {
   			window.parent.$('#InternalChatIFrame').contents().find('#MuteChatAlert').prop( "checked", true );
   		}
   		else
   		{
   			window.parent.$('#InternalChatIFrame').contents().find('#MuteChatAlert').prop( "checked", false );
   		}
...
<!-- CURRENT CHAT SPAN -->
	<body onLoad="MgrAgentAutoRefresh();" onUnLoad="clearInterval(rInt);" style="background-color: #f8f9fa!important;"><!-- DisplayMgrAgentChat(<?php echo $priority_chat; ?>); -->
...
<div class='container'>	
	...
	echo '<div class="row">';
		echo '<div class="col-8">';
			echo "<small><span class='arial_small' id='ActiveChatManager'>".$chat_managers_array[$priority_chat]."</span></small>";
		echo '</div>';
		echo '<div class="col-4 text-right">';
			echo "<small></font><span class='arial_small' id='ActiveChatStartDate'>".$chat_start_date_array[$priority_chat]."</span></small>";
		echo '</div>';
	echo '</div>';

	echo '<div class="row">';
		echo '<div class="col-3">';
			echo "<div class='scrolling_chat_display' id='AllActiveChats' style='border-radius: 5px;width:100%;height:300px'>\n";
			echo "<ul class='chatview'>";
			if (empty($chat_managers_array)) {
				echo "\t<li class='arial_bold'>"._QXZ("No Current Chats")."</li>\n";
			} else {
		...
		echo '<div class="col-9">';
			echo '<div class="row">';
				echo '<div class="col-12">';
					echo "\t<div class='scrolling_transcript' id='ActiveManagerChatTranscript' style='border-radius: 5px;width:100%;'></div><BR>\n";
				echo '</div>';
			echo '</div>';
////can use small_arial as smaller font

		echo "\t<div id='AllowAgentReplies' align='center' style='display:none;'>\n";
			echo '<div class="row">';
				echo '<div class="col-10">';
					echo "\t<textarea class='arial' rows='2' cols='65' style='border-radius: 5px;width:100%;' name='manager_message' id='manager_message' onkeypress='if (event.keyCode == 13) {SendMgrChatMessage();}'></textarea>";
				echo '</div>';
				echo '<div class="col-2">';
					echo "<button type='button' class='btn btn-primary btn' onClick='SendMgrChatMessage()'>Send</button>";
				echo '</div>';
			echo '</div>';
		echo "\t</div>\n";
...
echo '<br>';
		...
		echo '<div class="row">';
			echo '<div class="col-4">';
				echo "<button type='button' class='btn btn-success btn-sm btn-block' onClick=\"document.getElementById('AgentChatSpan').style.display='none'; document.getElementById('AgentNewChatSpan').style.display='block';LoadAvailableAgentsForChat('AllLiveAgents', 'agent');\">New Chat</button>";
			echo '</div>';
			echo '<div class="col-4">';
				echo "<span id='AgentEndChatSpan' style='display: none;'><button type='button' class='btn btn-danger btn-sm btn-block' onClick='EndAgentToAgentChat()'>End chat</button></span>";
			echo '</div>';
			echo '<div class="col-4">';
				echo "<span id='AgentAddChatSpan' style='display: none;'><button type='button' class='btn btn-info btn-sm btn-block' onClick=\"LoadAvailableAgentsForChat('AllLiveNonChatAgents', 'agent_to_add'); ToggleSpan('AllLiveNonChatAgents'); ToggleSpan('invitetochatbutton');\">Add Agent</button></span>";
			echo '</div>';
		echo '</div>';	

	echo '<div class="row" style="margin:5px;">';
		echo '<div class="col-5">';
			echo "<input type='checkbox' id='MuteChatAlert' name='MuteChatAlert' onchange='mutealertcheckbox();'><small> Mute Alerts</small>";
		echo '</div>';
		echo '<div class="col-6">';
			echo "<div id='AllLiveNonChatAgents' style='display: none;'></div>";
		echo '</div>';
		echo '<div class="col-1">';
			echo "<button type='button' id='invitetochatbutton' class='btn btn-info btn-sm' onClick=\"AddAgentToExistingChat();\" style='display: none;' disabled>Invite</button>";			
		echo '</div>';
	echo '</div>';
	echo '</div>';
	echo '</div>';
		...
		<div class='col-8'>
		<?php
		echo "<textarea class='arial' rows='8' style='border-radius: 5px;width:100%; name='agent_message' id='agent_message' onchange='update_chat_dropdowns();'></textarea>";		
		echo '</div>';
		echo "<div class='col-4'>";
		...
		echo "<div class='row'>";
			echo '<div class="col-6">';
			echo "<button type='button' id='startnewchat' class='btn btn-success btn-sm btn-block' onClick=\"CreateAgentToAgentChat();\" disabled>Start</button>";
			echo "</div>";
			echo '<div class="col-6">';
			echo "<button type='button' class='btn btn-danger btn-sm btn-block' onClick=\"document.getElementById('AgentChatSpan').style.display='block'; document.getElementById('AgentNewChatSpan').style.display='none';\">Back</button>";
			echo "</div>";
		echo "</div>";
